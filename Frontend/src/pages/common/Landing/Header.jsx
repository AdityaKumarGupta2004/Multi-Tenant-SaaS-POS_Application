...
const Header = () => {
  ...
  const renderNavLink = (link, isMobile = false) => (
    ...
    >
      {link.name}
      <span
        className={`absolute bottom-0 left-0 h-0.5 bg-primary transition-all duration-300 ${isMobile ? "hidden" : "w-0 group-hover:w-full"}`}
      ></span>
    </a>
  );
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? "bg-gray-900/70 backdrop-blur-lg border-b border-white/10" : "bg-transparent"}`}
    >
        ...
        <div className="flex items-center h-20">
          ...
          <div className="flex-1 hidden md:flex justify-end items-center space-x-4">
            <button
              ...
              className="relative font-medium transition-colors text-muted-foreground hover:text-primary group py-2 px-2"
            >
              Sign In
